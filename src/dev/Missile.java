...
public class Missile {
	...
	private GUI gui;
	private float x, y, width, height, startY, frames = 0, angle = 10f;
	...
	public Missile(GUI gui, float x, float y, float power) {
		this.gui = gui;
		this.x = x;
		this.y = y;
		this.startY = y;
	...
	public void tick() {
		if (!hitFloor) {
			...
			x = xSpeed * time;
			//y = (float) ((Physics.gravity * (time*time)) + (ySpeed*time) + startY);
			y = (float) startY - (ySpeed * time + ((-Physics.gravity / 2) * (time*time)));
			// System.out.println("X: " + x + " || Y:" + y + " " + gui.getDeltaTime());
		}
